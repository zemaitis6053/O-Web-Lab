...
function FixedAnime() {
	const elemTop = $('main').offset().top;//#aboutの位置まできたら
...
function setFadeElement(){
	const windowH = $(window).height();	//ウィンドウの高さを取得
	const scroll = $(window).scrollTop(); //スクロール値を取得
    
    //出現範囲の指定
	const contentsTop = Math.round($('#contact').offset().top);	//要素までの高さ四捨五入した値を取得
	const contentsH = $('#contact').outerHeight(true);	//要素の高さを取得
    
    //2つ目の出現範囲の指定
	const contentsTop2 = Math.round($('#footer').offset().top);	//要素までの高さ四捨五入した値を取得
	const contentsH2 = $('#footer').outerHeight(true);//要素の高さを取得
...
const windowwidth = window.innerWidth || document.documentElement.clientWidth || 0;
			var responsiveImage = [//PC用の画像
				{ src: './img/audi_01.jpg'},
				{ src: './img/audi_driving_seat02.jpg'},
				{ src: './img/audi_engine_cover.jpg'},
				{ src: './img/paints.jpg'}
			];
